...
public class PatientServiceTest {
    ...
    void createThenFindById() {
        Patient p = new Patient("P1", "Paul", "paul@atu.ie");
    ...
    @Test
    void deleteSuccess() {
        Patient p = new Patient("P1", "Bob", "bob@atu.ie");
    ...
    void updateNameSuccess() {
        Patient p = new Patient("P1", "Mike", "mike@atu.ie");
        when(repo.findByPatientId("P1")).thenReturn(Optional.of(p));
    ...
    void updateDOBSuccess() {
        ...
        when(repo.findByPatientId("P1")).thenReturn(Optional.of(p));
        when(repo.save(p)).thenReturn(p);
        Optional<Patient> updated = service.updateDOB("P1", "2000-01-01");
        assertTrue(updated.isPresent());
        assertEquals("2000-01-01", updated.get().getDOB());
    }
}
